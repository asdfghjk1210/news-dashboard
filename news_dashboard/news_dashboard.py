...
# 카테고리별 키워드 (한/영 혼합)
CATEGORIES = {
    "에너지/공급": ["전쟁", "분쟁", "산유국", "OPEC", "가스", "석유", "기름", "oil", "gas", "oil price", "LNG", "energy crisis", "중동", "이란", "이스라엘"],
    "에너지/인프라": ["발전소", "전력", "전기", "전기료", "전기 요금", "에너지", "energy", "항만", "LNG", "송전"],
    "시장/금융": ["금리", "환율", "주식", "채권", "물가", "인플레이션", "스태그플레이션", "interest", "stock", "exchange", "inflation", "rate hike", "연준", "Fed"],
    "생활/민생": ["전기료", "택시", "배달", "물류", "화물차", "민생", "요금", "장바구니", "물가", "Electricity bill", "Taxi", "Delivery", "Logistics"],
    "정책/대응": ["지원금", "보조금", "대책", "규제", "예산", "지자체", "policy", "subsidy", "tariff", "긴급", "보증", "특례"]
}
...
def build_weekly_chart_data(articles):
    """
    지난 7일 날짜별 × 카테고리별 기사 수 집계
    → Chart.js datasets 형식으로 반환
    """
    ...
    for cat, color in COLORS.items():
        datasets.append({
            ...
            "borderColor": color,
            "backgroundColor": color.replace(")", ", 0.08)").replace("rgb", "rgba") if "rgb" in color else color + "14",
            "tension": 0.4,
            "pointRadius": 3,
            "borderWidth": 2
        })
...
def build_gu_summaries(articles):
    """
    구별 기사를 카테고리별로 묶어 자동 요약 텍스트 생성
    → {"팔달구": {"에너지/공급": ["제목1", ...], ...}, ...}
    """
    gu_data = {gu: defaultdict(list) for gu in GU_KEYWORDS}
    # 수원 관련 전체 구글 KR 기사도 기본으로 표시
    gu_data["팔달구"]["에너지/공급"].append("(수집된 구별 기사 없음 — 전체 시 공통 동향 적용)")

    for a in articles:
        gu = classify_gu(a["제목"])
        if gu:
            cat = a["분류"]
            gu_data[gu][cat].append(a["제목"])

    # 각 구에 기사가 없으면 전체 TOP 이슈를 fallback으로 삽입
    ...
    for gu, color in GU_COLORS.items():
        cats = gu_data[gu]
        lines = []
        # 기사가 있는 카테고리 우선 표시 (최대 3개 항목)
        shown_cats = set()
        for cat, titles in cats.items():
            if cat == "기타":
                continue
            line_titles = titles[:2]
            lines.append({"cat": cat, "items": [t[:35] + ("…" if len(t) > 35 else "") for t in line_titles]})
            shown_cats.add(cat)
            if len(lines) >= 3:
                break
        # 기사 부족 시 전체 top 이슈로 보완
        if len(lines) < 2:
            for cat, titles in top_by_cat.items():
                if cat not in shown_cats and cat != "기타" and titles:
                    lines.append({"cat": cat, "items": [titles[0][:35] + "…"]})
                    if len(lines) >= 3:
                        break
...
def build_policy_summary(articles):
    """정책/대응 카테고리 기사에서 상위 이슈 추출"""
    policy_articles = [a for a in articles if a["분류"] == "정책/대응"][:6]
    energy_articles = [a for a in articles if a["분류"] in ("에너지/공급", "에너지/인프라")][:4]
    market_articles = [a for a in articles if a["분류"] == "시장/금융"][:3]
    life_articles   = [a for a in articles if a["분류"] == "생활/민생"][:3]
    return {
        "에너지": [a["제목"][:40] + "…" for a in energy_articles[:2]],
        "민생":   [a["제목"][:40] + "…" for a in life_articles[:2]],
        "산업":   [a["제목"][:40] + "…" for a in market_articles[:2]],
        "정책":   [a["제목"][:40] + "…" for a in policy_articles[:3]],
    }
...
def _gu_block_html(gu_summaries):
    ...
    for gu, color in GU_BORDER.items():
        data  = gu_summaries.get(gu, {"lines": []})
        lines = data.get("lines", [])
        inner = ""
        for line in lines:
            cat_html = f'<span style="font-size:9px;font-weight:500;color:{color};margin-right:4px">[{line["cat"]}]</span>'
            items_html = "<br>".join([f'<span style="font-size:9.5px;color:#475569">· {item}</span>' for item in line["items"]])
            inner += f'<div style="margin-bottom:4px">{cat_html}<br>{items_html}</div>'
        if not inner:
            inner = '<span style="font-size:9.5px;color:#94a3b8">수집된 기사 없음</span>'
        html += f'''
        <div style="border-left:3px solid {color};padding:4px 8px;margin-bottom:6px;background:#fafafa;border-radius:0 4px 4px 0">
          <div style="font-size:11px;font-weight:500;color:{GU_TEXT[gu]};margin-bottom:4px">{gu}</div>
          {inner}
        </div>'''
    return html
...
def _yt_tabs_html():
    ...
    for i, (group, channels) in enumerate(YOUTUBE_CHANNELS.items()):
        ...
        tab_buttons += f'<button class="ytbtn {active}" onclick="openTab(event,\'yt{i}\')">{group}</button>'
        items = ""
        for ch in channels:
            items += f'''
            <div style="display:flex;align-items:center;gap:6px;padding:5px 6px;background:#f8fafc;border-radius:4px;border:0.5px solid #e2e8f0;margin-bottom:4px">
              <iframe width="80" height="50" src="https://www.youtube.com/embed?listType=user_uploads&list={ch["id"]}" 
                frameborder="0" allowfullscreen style="border-radius:3px;flex-shrink:0"></iframe>
              <div>
                <div style="font-size:10px;font-weight:500;color:#1e40af">{ch["name"]}</div>
                <div style="font-size:9px;color:#64748b;line-height:1.4">{ch["desc"]}</div>
              </div>
            </div>'''
        tab_contents += f'<div id="yt{i}" style="display:{display}">{items}</div>'
    return tab_buttons, tab_contents

def _policy_html(policy):
    colors = {"에너지": "#10b981", "민생": "#3b82f6", "산업": "#f59e0b", "정책": "#8b5cf6"}
    text_colors = {"에너지": "#065f46", "민생": "#1e40af", "산업": "#92400e", "정책": "#5b21b6"}
    html = ""
    for label, items in policy.items():
        c  = colors.get(label, "#94a3b8")
        tc = text_colors.get(label, "#374151")
        ...
        html += f'''
        <div style="border-left:3px solid {c};padding:4px 8px;margin-bottom:6px">
          <div style="font-size:10px;font-weight:500;color:{tc};margin-bottom:2px">[{label}]</div>
          {items_html}
        </div>'''
    return html

def _news_rows_html(articles):
    rows = ""
    for a in articles[:50]:
        badge_color = "#3b82f6" if a["지역"] == "국내" else "#ef4444"
        cat_colors = {
            "에너지/공급": "#0ea5e9", "에너지/인프라": "#06b6d4",
            "시장/금융": "#10b981", "생활/민생": "#f59e0b",
            "정책/대응": "#8b5cf6", "기타": "#94a3b8"
        }
        cat_color = cat_colors.get(a["분류"], "#94a3b8")
        rows += f'''<tr>
          <td><span style="background:{badge_color};color:white;padding:2px 6px;border-radius:4px;font-size:9px">{a["지역"]}</span></td>
          <td><span style="background:{cat_color};color:white;padding:2px 6px;border-radius:4px;font-size:9px">{a["분류"]}</span></td>
          <td style="color:#374151">{a["언론사"]}</td>
          <td style="color:#64748b">{a["발행KST"].strftime("%m-%d %H:%M")}</td>
          <td><a href="{a["링크"]}" target="_blank" style="color:#3b82f6;text-decoration:none">{a["제목"][:55]}{"…" if len(a["제목"])>55 else ""}</a></td>
        </tr>'''
    return rows

def _econ_chart_js(econ_data, econ_dates):
    return f"""
    new Chart(document.getElementById('econChart'), {{
        ...
        options: {{
            ...
            scales:{{
                ...
                y:  {{ ticks:{{ font:{{ size:9 }} }}, title:{{ display:true, text:'환율', font:{{ size:9 }} }} }},
                y2: {{ position:'right', ticks:{{ font:{{ size:9 }} }}, title:{{ display:true, text:'유가/금', font:{{ size:9 }} }}, grid:{{ drawOnChartArea:false }} }}
            }}
        }}
    }});"""
...
def save_all(articles, econ_data, econ_dates):
    ...
    # ── 부분 HTML 생성 ─────────────────────────────────
    gu_block_html   = _gu_block_html(gu_summaries)
    yt_buttons, yt_contents = _yt_tabs_html()
    policy_html     = _policy_html(policy)
    news_rows       = _news_rows_html(articles)
    econ_chart_js   = _econ_chart_js(econ_data, econ_dates)
    top5_html       = "".join([
        f'<li style="margin-bottom:5px;font-size:11px"><span style="background:#ef4444;color:white;padding:1px 5px;border-radius:3px;font-size:9px;margin-right:4px">{a["언론사"]}</span>'
        f'<a href="{a["링크"]}" target="_blank" style="color:#1e40af;text-decoration:none">{a["제목"][:60]}{"…" if len(a["제목"])>60 else ""}</a></li>'
        for a in articles[:5]
    ])

    # 종합 현황 텍스트 — 카테고리 기사 수 기반 자동 생성
    ...
    issues_html = "".join([
        f'<div style="margin-bottom:4px;font-size:10px;color:#374151">· [{cat}] 관련 기사 <strong>{cnt}건</strong> — 주요 이슈 집중 모니터링 필요</div>'
...
<title>수원시 구별 중동 상황 및 분석 대시보드</title>
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
<style>
  *{{box-sizing:border-box;margin:0;padding:0}}
  body{{font-family:'Malgun Gothic','Apple SD Gothic Neo',sans-serif;background:#f0f2f5;color:#1e293b}}
  .wrap{{max-width:1400px;margin:0 auto;padding:10px}}
  /* 헤더 */
  .main-hdr{{background:#1e3a8a;color:white;padding:10px 16px;border-radius:10px 10px 0 0;display:flex;justify-content:space-between;align-items:center;margin-bottom:0}}
  .main-hdr h1{{font-size:15px;font-weight:500}}
  .main-hdr .ts{{font-size:10px;opacity:.75}}
  /* 탭 네비 */
  .nav-tabs{{background:#1e40af;display:flex;gap:2px;padding:0 16px}}
  ...
  .nav-tabs a:hover,.nav-tabs a.active{{color:white;border-bottom:2px solid white}}
  /* 그리드 */
  ...
  /* 반응형 */
  @media(max-width:900px){{.r3,.r3b{{grid-template-columns:1fr}}.r2{{grid-template-columns:1fr}}}}
</style>
</head>
<body>
<div class="wrap">
  ...
  <div class="main-hdr">
    ...
    <span class="ts">마지막 업데이트: {now_str}</span>
  </div>
  <div class="nav-tabs">
  ...
  <div class="row r3">
    ...
    <div class="card">
      <div class="card-hdr">
        수원시 구별 상황 유형
        <span style="font-size:9px;color:#64748b">RSS 자동 분류</span>
      </div>
      {gu_block_html}
    </div>
    ...
    <div class="card">
      ...
      <div class="chart-wrap" style="height:160px"><canvas id="kwChart"></canvas></div>
      <div style="margin-top:8px">
        <div class="card-hdr" style="margin-top:6px">주요 경제 지표 (USD/KRW · WTI · 금값)</div>
        <div class="chart-wrap" style="height:160px"><canvas id="econChart"></canvas></div>
      </div>
    </div>
  ...
  <div class="row r2">
    ...
    <div class="sec-wrap">
      <div class="sec-hdr">
        종합 상황 및 정책 제안
        <a href="{OUTPUT_XLSX}" class="dl-btn">종합 보고서 (xlsx) 다운로드</a>
      </div>
      <div class="sec-body">
        <div style="font-size:10px;font-weight:500;color:#374151;margin-bottom:6px">▶ 수원시 도 &gt; 종합 현황 및 문제점 (RSS 자동 집계)</div>
        {issues_html}
      ...
      <div class="sec-body">
        <div class="row r3b">
          <div>
            ...
            {''.join([f'<div style="font-size:9px;background:#eff6ff;border-left:2px solid #3b82f6;padding:3px 6px;margin-bottom:3px;border-radius:0 3px 3px 0"><a href="{a["링크"]}" target="_blank" style="color:#1e40af;text-decoration:none">{a["제목"][:38]}{"…" if len(a["제목"])>38 else ""}</a></div>' for a in articles if a["지역"]=="국외"][:6])}
          </div>
          <div>
            ...
            {''.join([f'<div style="font-size:9px;background:#f0fdf4;border-left:2px solid #10b981;padding:3px 6px;margin-bottom:3px;border-radius:0 3px 3px 0"><a href="{a["링크"]}" target="_blank" style="color:#065f46;text-decoration:none">{a["제목"][:38]}{"…" if len(a["제목"])>38 else ""}</a></div>' for a in articles if a["지역"]=="국내"][:6])}
          </div>
          <div>
            ...
            <ol style="padding-left:12px">{top5_html}</ol>
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="sec-wrap">
    <div class="sec-hdr">
      국내외 언론 보도 모니터링 데이터 (크롤링 데이터)
      <a href="{OUTPUT_XLSX}" class="dl-btn">크롤링 데이터 (엑셀.xlsx) 다운로드</a>
    </div>
    <div style="overflow-x:auto">
      <table>
        <thead>
          <tr>
            <th>지역</th><th>분류</th><th>언론사</th>
            <th>보도일(KST)</th><th>제목 (클릭 시 원문)</th>
          </tr>
        </thead>
...
new Chart(document.getElementById('kwChart'), {{
  ...
  data: {{
    ...
    datasets: {json.dumps(kw_datasets)}
  }},
  options: {{
    responsive:true, maintainAspectRatio:false,
